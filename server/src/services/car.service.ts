...
export function filterCars(filters: {
  ...
  return getAllCars().filter((car) => {
    ...
    if (filters.bodyType && filters.bodyType !== 'any' && car.bodyType !== filters.bodyType) return false;
    if (filters.fuelType && filters.fuelType !== 'any' && car.fuelType !== filters.fuelType) return false;
    if (filters.transmission && filters.transmission !== 'any' && car.transmission !== filters.transmission) return false;
    if (filters.seatingCapacity && car.seatingCapacity < filters.seatingCapacity) return false;
    return true;
  });
}

function scoreCar(car: Car, answers: QuestionnaireAnswers): { score: number; reasons: string[] } {
  let score = 50;
  ...
  if (answers.bodyType === 'any' || car.bodyType === answers.bodyType) {
    score += 10;
    if (answers.bodyType !== 'any') reasons.push(`Matches ${answers.bodyType} preference`);
  }

  if (answers.fuelType === 'any' || car.fuelType === answers.fuelType) {
    score += 8;
    if (answers.fuelType !== 'any') reasons.push(`${car.fuelType} engine as preferred`);
  }

  if (answers.transmission === 'any' || car.transmission === answers.transmission) {
    score += 7;
    if (answers.transmission !== 'any') reasons.push(`${car.transmission} transmission`);
  }
  ...
  for (const priority of answers.priorities) {
    ...
    if (priority === 'mileage' && car.mileage >= 18) score += 5;
    if (priority === 'safety' && car.safetyRating >= 4) score += 5;
    if (priority === 'features' && car.features.length >= 5) score += 5;
    if (priority === 'performance' && car.engineCC >= 1400) score += 5;
  }

  return { score: Math.min(score, 100), reasons: [...new Set(reasons)] };
}

export function getRecommendations(answers: QuestionnaireAnswers): RecommendationResult[] {
...
export function compareCars(carIds: string[]): CompareResponse {
  const cars = carIds.map((id) => getCarById(id)).filter(Boolean) as Car[];

  const comparison: Record<string, Record<string, number | string>> = {
    price: {},
    mileage: {},
    safetyRating: {},
    engineCC: {},
